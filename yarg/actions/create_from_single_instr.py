...
class CreatePatternFromSelectedInstructionHandler(kw.action_handler_t):
    def __init__(self):
        super().__init__()
    ...
    def activate(self, ctx):
        ...
        if not code:
            kw.warning(f"[YarG] Selected instruction read failed (S:{hex(ea)} ; E:{hex(ea + size)})")
            return 0
        ...
        with current_database() as db:
            result = create_pattern_from_code(md, code, ea, settings, db=db)
        ...
        try:
            yar_rule = build_code_rule(ea, pattern, result.annotations, bitness, VAR_NAME, "instr")
        except YaraOutputError as exc:
            kw.warning(f"[YarG] {exc}")
            return 0

        show_yara_rule(f"YarG instruction {ea:0{bitness // 4}X}", yar_rule)
        return 0
